...
void mark(word* block) {
  // color white blocks black
  if (Color(block[0]) == White) {
    block[0] = Paint(block[0], Black);
  }

  // mark cons references recursively
...
void sweepPhase() {
  ...
  int length = 0;
  do {
    word *block = (word*)&heap[i];

    // orphan block? skip!
    if (Length(block[0]) == 0) continue;

    length = Length(block[0]);

    if (Color(block[0]) == Black)
      block[0] = Paint(block[0], White);
    else if (Color(block[0]) == White) {
      block[0] = Paint(block[0], Blue);

      //-- EXERCISE 10.3 BEGIN --//
      int offset = Length(block[0]) + 1;

      // does offset block exist?
      ...
      // To complete exercise 10.4, one would simply use
      // a loop that runs through the freelist more than
      // one time to collect all fragments together.

      block[1] = (word) freelist;
      freelist = (word*) &block[0];
    }
  } while ((i += length + 1) < HEAPSIZE);
}
