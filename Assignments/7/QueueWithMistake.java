...
class QueueWithMistake {
  ...
  private static void runThreads(final int threads, final Queue queue) {
    final int iterations = 10000000; // Increase this constant if program does not run out of memory.
...
class SentinelLockQueue implements Queue {
  ...
  public synchronized int get() {
    ...
    Node first = head;
    head = first.next;
    // Solution: because the dummy node always has a reference to
    // the first element in the queue, the elements removed from
    // the queue can never be garbage collected.
    // We can solve this problem by removing the dummy objects
    // reference to next, thus severing the link.
    if (dummy.next != null) dummy.next = null;
    return head.item;
  }
}
