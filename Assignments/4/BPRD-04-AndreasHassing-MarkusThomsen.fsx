...
in add 2 5 end");; // => HigherFun.value = Int 7

run (fromString @"let add x = let f y = x+y in f end
in let addtwo = add 2
    in addtwo 5 end
end");; // => HigherFun.value = Int 7
...
// Following are some examples of the functionality (working).
run (fromString @"let add x = fun y -> x+y in add 2 5 end");;
    // => HigherFun.value = Int 7

run (fromString @"let y = 22 in fun z -> z+y end");;
    // => HigherFun.value = Clos ("z",Prim ("+",Var "z",Var "y"),[("y", Int 22)])
