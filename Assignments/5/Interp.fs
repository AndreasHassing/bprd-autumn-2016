...
and eval e locEnv gloEnv store : int * store = 
    ...
    //Excersice 7.4     We copy the access operation.
    | PreInc acc     -> let (loc, store1) = access acc locEnv gloEnv store
                        // We get the value stored at the access position, and increment it by one.
                        let value = getSto store1 loc + 1
                        // We return a tuple of the value, and the store produce by the setSto function.
                        (value, setSto store1 loc value)
                        // Same as above, except decrementing instead of incrementing.
